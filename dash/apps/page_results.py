...
def generate_table(dataframe, max_rows=10):
    ...
    #return html.Table(
    #    # Header
    #    [html.Tr([html.Th(col) for col in dataframe_text.columns])] +

    #    # Body
    #    [html.Tr([
    #        #html.Td('{}'.format(dataframe.iloc[i][col])) for col in dataframe.columns
    #        html.Td(dataframe_text.iloc[i][col]) for col in dataframe_text.columns
    #    ]) for i in range(min(len(dataframe_text), max_rows))],
    #)
...
1. Novel coronavirus diagnostic test data comes from either the FDA or FindDx. 
  - [The FDA publishes data on coronavirus diagnostic tests given Emergency Use Authorization](https://www.fda.gov/emergency-preparedness-and-response/mcm-legal-regulatory-and-policy-framework/emergency-use-authorization)
...
def set_test_options(selected_company):
    ...
    company_cond = df.company.str.contains(selected_company, regex=False)
    df_company_filtered = df[company_cond]
...
def generate_graphs(dropdown_value, dropdown_test_value, slider_prevalence_value):
    ...
    if dropdown_value is None or dropdown_test_value is None:
        ...
        #table = generate_table(df[0:200])
    else:
        dff = df[df.company.str.contains(dropdown_value, regex=False) &
                 df.test_name.str.contains(dropdown_test_value, regex=False)]
        markdown_text = 'testing positive'
